...
// expand the file object to include the records from the file_dbxref table
$options = array('return_array' => 1);
$file = chado_expand_var($file, 'table', 'file_dbxref', $options);
$file_dbxrefs = $file->file_dbxref;

$references = array();
if (count($file_dbxrefs) > 0 ) {
  foreach ($file_dbxrefs as $file_dbxref) {    
    $references[] = $file_dbxref->dbxref_id;
  }
}

if(count($references) > 0){ ?>
  <div class="tripal_file-data-block-desc tripal-data-block-desc">This file is also available in the following databases:</div><?php 
  ...
  $table = array(
    ...
    'rows' => $rows,
    'attributes' => array(
      'id' => 'tripal_file-table-references',
